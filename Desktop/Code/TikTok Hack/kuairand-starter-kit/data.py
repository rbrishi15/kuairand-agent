...
def build_vocabs(tr):
    """构建 5 个域的词表，只用 train。被 encode() 和 ssl_pretrain.py 共用 ——
    保证 video_id 等词表 id 在两边完全对齐，SSL 学出的 embedding 才能直接
    对上 FM 里对应的行。"""
    ...
    return vocabs, edges, raw

def field_layout(tr):
    """Returns (vocabs, field_dims, offsets). Used to find which rows of V
    belong to the video_id field: offsets[vi] : offsets[vi]+field_dims[vi]."""
    vocabs, edges, raw = build_vocabs(tr)
    field_dims = [len(v) + 1 for v in vocabs]
    offsets = np.cumsum([0] + field_dims[:-1]).astype(np.int32)
    return vocabs, field_dims, offsets
